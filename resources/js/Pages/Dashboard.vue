<template>
    ...
    <MainLayout>
        <template #header>
<!--            <h2 class="sm:font-semibold sm:text-xl sm:text-gray-800 sm:leading-tight text-sm" ></h2>-->
            <h2 class="sm:font-semibold sm:text-xl sm:text-gray-800 sm:leading-tight text-sm" v-html="hiName"></h2>
        </template>
                ...
                <div>
                    <div class="flex justify-between items-center items_flex p-4">
                        ...
                        <div class="content_item ml-8 sm:w-3/4 w-full">
                            ...
                            <p class="italic text-base">{{$page.props.auth.user.birthday !== null ? $page.props.auth.user.birthday  : 'Ваша дата рождения'}}</p>
                            <div class="py-2 text-justify" v-html="$page.props.auth.user.about !== null ? $page.props.auth.user.about : '<p>Информация о Вас</p>'"></div>
                        </div>
                    </div>
                        ...
                        <div class="scroll">
                            <div class="grid md:grid-cols-2 sm:grid-cols-1 gap-3 my-3" v-if="holidays.length > 0">
                                ...
                                <div class="flex justify-center" v-for="item in holidays">
                                    <div class="no_happy">
                                        <img class="img" :src="item.image == null ? img : item.image" :alt="item.title">
    ...
    computed: {
        hiName() {
            if(window.innerWidth < 400){
                return `Приветствуем, <br> ${this.$page.props.auth.user.name} !`
            }else{
                return `Приветствуем, ${this.$page.props.auth.user.name} !`
            }
        }
    },
    methods: {
